...
select location, population, max(total_cases) HighestInfectionCount, (max(total_cases)/population)*100 HighestInfectionPercentage
from coviddeath
...
select location, population, max(total_deaths) HighestDeathCount, (max(total_deaths)/population)*100 HighestDeathPercentage
from coviddeath
...
with countries as (
select continent, location, population, max(total_deaths) HighestDeathCount, (max(total_deaths)/population)*100 HighestDeathPercentage
from coviddeath
where continent is not null 
group by continent, location, population 
having (max(total_deaths)/population)*100 is not null
)
select continent, sum(population) population, sum(HighestDeathCount) HighestDeathCount, (sum(HighestDeathCount)/sum(population))*100 HighestDeathPercentage
from countries
group by continent 
order by HighestDeathPercentage desc;

-- Global Number Date by Date
select date, sum(new_cases) new_cases, sum(new_deaths) new_deaths, (sum(new_deaths)/sum(new_cases))*100 DeathPercentage
from coviddeath
where continent is not null
group by date
...
select *, (accumulative_vaccinations/population)*100 PercentPopulationVaccinated
from PopvsVac;
